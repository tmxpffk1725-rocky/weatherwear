...
- **추천**: LLM 우선 + 룰 기반 폴백. 옷장·피부톤·핏·하루범위(일 최고/최저·강수확률) 반영. 상황별 복장 규칙 자동 강제(운동=운동복·운동화만 — 프롬프트+룰 폴백 `SPORT_RANGE`). 카드는 항상 정확, LLM 설명 문구는 가끔 슬립(허용).
- **날씨/추천 회복력**: 날씨 서버는 격자별 마지막 성공 데이터를 무기한 보관(JSON 영속화)해 기상청 실패 시 stale+asOf로 제공하고, 일시 오류(순간 429·타임아웃)는 1.5초 후 1회만 재시도. 프론트는 stale이면 "○분 전 날씨" 표시, 서버까지 실패 시 기기 캐시→수동 기온 선택. 추천(LLM/네이버)은 재시도 없이 즉시 폴백(쿼터 절약) 유지.
